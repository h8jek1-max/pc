...
$skipCsrf = in_array($action, [
    ...
    if ($mod === 'dbpr') {
        ...
        switch ($action) {
            ...
            // 2. STRÁNKOVANÝ VÝPIS DAT S FILTRACÍ PRO PROHLÍŽEČ (Nyní bleskově přes proudové hledání)
            case 'db_get_data':
               // $vysledekHledani = $db->run($tabulka, 'select', ['ignore_idx' => true]);

            // 2. STRÁNKOVANÝ VÝPIS DAT S FILTRACÍ (SELECT VČETNĚ LINKŮ)
                $tabulka = (string)($data['table'] ?? '');
                $stranka = (int)($data['page'] ?? 1);
                $limit   = (int)($data['limit'] ?? 20);
                $hledanySloupec = (string)($data['search_col'] ?? '');
                $hledanaHodnota = (string)($data['search_val'] ?? '');

                if (empty($tabulka)) throw new Exception("Chybí název tabulky.");

                $params = [];
                if (!empty($hledanySloupec) && !empty($hledanaHodnota)) {
                    $params['where'] = [$hledanySloupec => $hledanaHodnota];
                }

                $vsechnaData = $db->run($tabulka, 'select', $params);
                
                $celkemPolozek = count($vsechnaData);
                $offset = ($stranka - 1) * $limit;
                $orezanaData = array_slice($vsechnaData, $offset, $limit);
