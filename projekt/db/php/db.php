...
class DB {
    ...
    private function provedUpsertZapis(string $table, array $params): string {
        ...
        $idStr = (string)$dataZaznamu['id'];
		        ...
		        // --- JEDNOTNÉ SOUBOROVÉ UKLÁDÁNÍ: PŘEVOD POLÍ NA JSON TEXT ---
        foreach ($dataZaznamu as $sloupec => &$hodnota) {
            // Ignorujeme systémové klíče
            if (strpos($sloupec, '_') === 0 || $sloupec === 'id') continue;

            if (is_array($hodnota)) {
                // Skutečné PHP pole ["input", "dbpr"] překlopíme na text '["input","dbpr"]'
                $hodnota = json_encode($hodnota, JSON_UNESCAPED_UNICODE);
            } elseif (is_string($hodnota)) {
                $trimmed = trim($hodnota);
                // Pokud z front-endu přišel text v uvozovkách se single-quotes '[ "input" ]',
                // normalizujeme jej na validní JSON formát se double-quotes
                if (strpos($trimmed, '[') === 0 && substr($trimmed, -1) === ']') {
                    $upravenyText = str_replace("'", '"', $trimmed);
                    // Ověříme, zda je text validním JSONem
                    if (json_decode($upravenyText) !== null) {
                        $hodnota = $upravenyText;
                    }
                }
            }
        }
...
return $idStr;
     ...
     */
    /**
     * UNIVERZÁLNÍ PROUDOVÝ VYHLEDÁVAČ: Podporuje texty, pole i textové JSON řetězce
     */
    private function provedFulltextSearch(string $table, array $params): array {
        ...
        // Příprava čistého klíčového slova (odstraníme uvozovky a závorky, pokud přišly z našeptávače)
        $cisteHledaneSlovo = $hledanaHodnota;
        if (strpos($hledanaHodnota, '[') === 0 && substr($hledanaHodnota, -1) === ']') {
            // Pokud přišel z JS text '["input","dbpr"]', vytáhneme z něj jen to podstatné slovo
            $rozbaleno = json_decode(str_replace("'", '"', $hledanaHodnota), true);
            if (is_array($rozbaleno) && !empty($rozbaleno)) {
                $cisteHledaneSlovo = (string)$rozbaleno[0]; // Vezmeme první prvek pro porovnání
            }
        }
        $hledatMalymi = mb_strtolower($cisteHledaneSlovo, 'UTF-8');

        $segmenty = glob(DB_PATH . "/json/{$table}_*.db") ?: [];
        sort($segmenty);

        foreach ($segmenty as $cestaSegmentu) {
            if (!file_exists($cestaSegmentu)) continue;

            $fsData = new SplFileObject($cestaSegmentu, 'r');
            while (!$fsData->eof()) {
                ...
                // Pokud je vyhledávací políčko prázdné, bereme řádek rovnou (pro základní výpis)
                ...
                if (!empty($hledanySloupec)) {
                    // ─── VARIANTA A: VYHLEDÁVÁNÍ V KONKRÉTNÍM SLOUPCI ───
                    if (isset($radek[$hledanySloupec])) {
                        $obsahBunky = mb_strtolower((string)$radek[$hledanySloupec], 'UTF-8');
                        if (strpos($obsahBunky, $hledatMalymi) !== false) {
                            $filtrovanaData[] = $radek;
                        }
                    }
                } else {
                    // ─── VARIANTA B: GLOBÁLNÍ VYHLEDÁVÁNÍ NAPŘÍČ TABULKOU ───
