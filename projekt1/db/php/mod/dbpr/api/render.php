    ...
    <div class="dbpr-panel">
        <div class="dbpr-header">
            <div class="form-group">
                <label>📋 Tabulka</label>
                <select id="dbpr_table_select" class="input" onchange="DBPR.loadTableData()">
            ...
            <div class="form-group">
                ...
                <div class="dbpr-autocomplete" id="dbpr_val_ac">
                    <input type="text" id="dbpr_val_input" class="input dbpr-autocomplete-input" placeholder="Hledat hodnotu..." autocomplete="off">
...
window.DBPR = {
    ...
    async fetchData() {
        try {
            ...
            const data = await PC.api('dbpr', 'db_get_data', {
    ...
    renderTable(records, total) {
        ...
        records.forEach(record => {
            ...
            this.columns.forEach(col => {
                ...
                td.innerHTML = `<span style="cursor:pointer;" onclick="DBPR.openEditModal('${_esc(String(record.id))}', '${_esc(col)}', ${JSON.stringify(_esc(display))})">${this.truncate(display, 50)}</span>`;
                row.appendChild(td);
            });
    ...
    async searchValues(query, resultsDiv) {
        ...
        try {
            ...
            resultsDiv.innerHTML = values.map(v => `<div class="dbpr-autocomplete-item" onclick="DBPR.selectValue('${_esc(String(v))}')">${this.truncate(String(v), 50)}</div>`).join('');
            resultsDiv.classList.add('show');
        } catch (e) {
            resultsDiv.classList.remove('show');
        }
